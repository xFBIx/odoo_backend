...
def validate_signup_data(data):
    ...
    if "role" in data and data["role"] not in ["Customer", "Librarian", "Admin"]:
...
def signup(request):
    ...
    if errors:
        return Response(
            {"status": "error", "errors": errors}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        return Response(
            {"status": "success", "user": user_data, "tokens": tokens},
    ...
    try:
        ...
        if created:
            UserProfile.objects.create(
                auth_user=user, name=f"{first_name} {last_name}".strip(), role=role
            )

        tokens = get_tokens_for_user(user)
        user_data = get_user_data(user)

        response_data = {"status": "success", "user": user_data, "tokens": tokens}
